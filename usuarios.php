...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    } # Endpoint = Consultar por Correo y Password 
    elseif (isset($_GET['usuario']) && isset($_GET['password'])) {
        $usuario = $_GET['usuario'];
        $password = $_GET['password'];
        $dbconn->query("SET NAMES 'UTF8'");
        $sql = "SELECT id_usuario,usuario,password FROM usuarios WHERE usuario='$usuario' and password='$password'";
        $stmt = $dbconn->prepare($sql);
        $stmt->execute();
        header("Access-Control-Allow-Origin: *");
        header("Access-Control-Allow-Headers: *");
        ...
        if (!$sql) {
            ...
            $stmt->setFetchMode(PDO::FETCH_ASSOC);
            $salidadatos = json_encode($stmt->fetchObject());
            //echo '{"userData": "' . $salidadatos . '"}';
            echo $salidadatos ;
        }
    } else {
